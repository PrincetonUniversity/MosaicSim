...
PreservedAnalyses AlwaysInlinerPass::run(Module &M, ModuleAnalysisManager &) {
  ...
  for (Function &F : M) {
    if (!F.isDeclaration()  &&  isInlineViable(F)) {
      ...
      for (User *U : F.users()) {
        if(F.getName().str().find("_kernel_") != std::string::npos) {
          errs() << "User of Kernel: " << F.getName().str();
        }
...
static RegisterPass<AlwaysInlinerLegacyPass> X("alwaysinline", "Always Inline Pass");
INITIALIZE_PASS_BEGIN(AlwaysInlinerLegacyPass, "whatever",
                      "Inliner for always_inline functions", false, false)
INITIALIZE_PASS_DEPENDENCY(AssumptionCacheTracker)
INITIALIZE_PASS_DEPENDENCY(CallGraphWrapperPass)
INITIALIZE_PASS_DEPENDENCY(ProfileSummaryInfoWrapperPass)
INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfoWrapperPass)
INITIALIZE_PASS_END(AlwaysInlinerLegacyPass, "",
                    "Inliner for always_inline functions", false, false)
