...
public:
  //todo find out why ID is static. 
  static char ID;
  Graph depGraph;
...
bool GraphGen::runOnFunction(Function &func) {
  ...
  if (isKernelFunction(func)) {
    assert(!found_kernel);
    found_kernel = true;
    GraphGen::constructGraph(func);
    errs() << "Done constructing graph\n";// << func.getName().str() << "\n";
    GraphGen::addDataEdges(func);
    GraphGen::addControlEdges(func);
    GraphGen::addPhiEdges(func);
    GraphGen::detectFunctions(func);
    //analyzeLoop();
    GraphGen::visualize();
    GraphGen::exportGraph();
        ...
        for (Use &use : inst.operands()) {
          ...
          if(Function *f = dyn_cast<Function>(v)) {
            if(f->getName().str().find("desc_supply_produce") != std::string::npos || f->getName().str().find("desc_supply_special_produce") != std::string::npos || f->getName().str().find("desc_special_supply_produce") != std::string::npos) {              
              ...
              n->itype = PARTIAL_BARRIER;
            }

            else if ((f->getName().str().find("decadesTF_matmul") != std::string::npos) || (f->getName().str().find("decadesTF_sdp") != std::string::npos) || (f->getName().str().find("decadesTF_conv2d_layer") != std::string::npos) || (f->getName().str().find("decadesTF_dense_layer") != std::string::npos) ||  (f->getName().str().find("decadesTF_bias_add") != std::string::npos)) {
              //errs() << "[ACCELERATOR]"<< *i << "\n";
              n->itype = ACCELERATOR;
              //assert(false);
            }
            ...
            else if (f->getName().str().find("dec_atomic_fetch_add_float_async") != std::string::npos) {
              //errs() << "[STVAL]"<< *i << "\n";
              n->itype = ATOMIC_FADD;
              //n->itype = LD;
            }
            else if (f->getName().str().find("dec_atomic_fetch_add") != std::string::npos) {
              //errs() << "[STVAL]"<< *i << "\n";
              n->itype = ATOMIC_ADD;
              //n->itype = LD;
            }
            else if (f->getName().str().find("dec_atomic_compare_exchange") != std::string::npos) {
              //errs() << "[STVAL]"<< *i << "\n";
              n->itype = ATOMIC_CAS;
              //n->itype = LD;
            }
            else if (f->getName().str().find("dec_atomic_fetch_min_async") != std::string::npos) {
              //errs() << "[STVAL]"<< *i << "\n";
              n->itype = ATOMIC_MIN;
              //n->itype = LD;
            }
