...
namespace {
  struct RecordDynamicInfo : public ModulePass {
    ...
    Function *print_partial_barrier;
    ...
    bool runOnModule(Module &M) override {
      ...
      print_matmul = (Function *) M.getOrInsertFunction("print_matmul", Type::getVoidTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getFloatPtrTy(ctx), Type::getFloatPtrTy(ctx), Type::getFloatPtrTy(ctx)).getCallee();

      print_sdp = (Function *) M.getOrInsertFunction("print_sdp", Type::getVoidTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getFloatPtrTy(ctx), Type::getFloatPtrTy(ctx), Type::getFloatPtrTy(ctx)).getCallee();

      print_bias = (Function *) M.getOrInsertFunction("print_bias", Type::getVoidTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getFloatPtrTy(ctx), Type::getFloatPtrTy(ctx), Type::getFloatPtrTy(ctx)).getCallee();

      print_conv2d_layer = (Function *) M.getOrInsertFunction("print_conv2d_layer", Type::getVoidTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getInt1Ty(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getInt1Ty(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getFloatPtrTy(ctx), Type::getFloatPtrTy(ctx), Type::getFloatPtrTy(ctx)).getCallee();

      print_dense_layer = (Function *) M.getOrInsertFunction("print_dense_layer", Type::getVoidTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx), Type::getFloatPtrTy(ctx), Type::getFloatPtrTy(ctx), Type::getFloatPtrTy(ctx)).getCallee();
      print_partial_barrier = (Function *) M.getOrInsertFunction("print_partial_barrier", Type::getVoidTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt8PtrTy(ctx), Type::getInt32Ty(ctx), Type::getInt32Ty(ctx)).getCallee();
    ...
    void instInspect(Function &F) {
      for (inst_iterator iI = inst_begin(F), iE = inst_end(F);iI != iE; iI++) {
        ...
        //print out the memory traces
          ...
          for (Use &use : inst->operands()) {
            ...
            if(Function *f = dyn_cast<Function>(v)) {
              ...
              if(f->getName().str().find("supply_consume") != std::string::npos || f->getName().str().find("dec_atomic_fetch_add_float_async") != std::string::npos || f->getName().str().find("dec_atomic_fetch_add") != std::string::npos ||  f->getName().str().find("dec_atomic_compare_exchange") != std::string::npos || f->getName().str().find("dec_atomic_fetch_min_async") != std::string::npos ||  f->getName().str().find("desc_supply_alu_rmw_cas") != std::string::npos ||  f->getName().str().find("desc_supply_alu_rmw_fetchmin") != std::string::npos || f->getName().str().find("desc_supply_alu_rmw_fetchadd_float") != std::string::npos) {
		...
		/* Find a way to get the first address for each input matrix */
                Value *ktype, *rdir, *rowsa_val, *colsa_val, *rowsb_val, *colsb_val,
		  *batch_val, *castI, *name_val, *acc_name_val, *A_ptr, *B_ptr, *C_ptr;
                IRBuilder<> Builder(inst);
                DataLayout* dl = new DataLayout(mod);
                LLVMContext& ctx = mod->getContext();
                ...
                A_ptr    =(cinst->getArgOperand(5));
                B_ptr    =(cinst->getArgOperand(6));
                C_ptr    =(cinst->getArgOperand(7));
		...
		rdir = Builder.CreateGlobalStringPtr(RUN_DIR);
                ...
                Value* args[] = {name_val, ktype, rdir, acc_name_val,
				 rowsa_val, colsa_val, rowsb_val, colsb_val,
				 batch_val, A_ptr, B_ptr, C_ptr};
              ...
              */
                ...
                Value *ktype, *rdir, *working_mode, *size, *name_val, *acc_name_val, *A, *B, *out;
                IRBuilder<> Builder(inst);
                DataLayout* dl = new DataLayout(mod);
                LLVMContext& ctx = mod->getContext();
                working_mode = (cinst->getArgOperand(0));
                size = (cinst->getArgOperand(1));
                A = (cinst->getArgOperand(2));
                B = (cinst->getArgOperand(3));
                out = (cinst->getArgOperand(4));
		...
		rdir = Builder.CreateGlobalStringPtr(RUN_DIR);
                ...
                name_val = Builder.CreateGlobalStringPtr(namestr);
                acc_name_val = Builder.CreateGlobalStringPtr(acc_name);
                Value* args[] = {name_val,ktype,rdir,acc_name_val,working_mode, size, A, B, out};
	      ...
	      // 			      int tid, int num_threads)
              else if(f->getName().str().find("decadesTF_bias_add") != std::string::npos) {
                Value *ktype, *rdir, *acc_name_val, *name_val,  *batch, *size, *in, *bias, *out;
                IRBuilder<> Builder(inst);
                DataLayout* dl = new DataLayout(mod);
                LLVMContext& ctx = mod->getContext();
                batch = (cinst->getArgOperand(0));
                size = (cinst->getArgOperand(1));
                in = (cinst->getArgOperand(2));
                bias = (cinst->getArgOperand(3));
                out = (cinst->getArgOperand(4));
		...
		rdir = Builder.CreateGlobalStringPtr(RUN_DIR);
                ...
                std::string namestr = std::to_string(findID(inst));
                std::string acc_name("decadesTF_bias_add");
                //instr id
                name_val = Builder.CreateGlobalStringPtr(namestr);
                acc_name_val = Builder.CreateGlobalStringPtr(acc_name);
                Value* args[] = {name_val, ktype, rdir, acc_name_val, batch, size, in, bias, out};
                CallInst* cI = Builder.CreateCall(print_bias, args);
              }
...
batch -> batch_size
              */
                ...
                Value *ktype, *rdir, *name_val, *acc_name_val, *batch, *in_channels, *in_height, *in_width, *out_channels, *filter_height, *filter_width, *zero_pad, *vert_conv_stride, *horiz_conv_stride, *pooling, *pool_height, *pool_width, *vertical_pool_stride, *horizontal_pool_stride, *in, *filters, *out;
                IRBuilder<> Builder(inst);
                DataLayout* dl = new DataLayout(mod);
                LLVMContext& ctx = mod->getContext();

                batch=(cinst->getArgOperand(0));in_channels=(cinst->getArgOperand(3)); in_height=(cinst->getArgOperand(1));in_width=(cinst->getArgOperand(2));out_channels=(cinst->getArgOperand(6));filter_height=(cinst->getArgOperand(4));filter_width=(cinst->getArgOperand(5));zero_pad=(cinst->getArgOperand(7));vert_conv_stride=(cinst->getArgOperand(8));horiz_conv_stride=(cinst->getArgOperand(9));pooling=(cinst->getArgOperand(13));pool_height=(cinst->getArgOperand(15));pool_width=(cinst->getArgOperand(16));vertical_pool_stride=(cinst->getArgOperand(17));horizontal_pool_stride=(cinst->getArgOperand(18));in=(cinst->getArgOperand(25));in=(cinst->getArgOperand(26));in=(cinst->getArgOperand(28));
		...
		rdir = Builder.CreateGlobalStringPtr(RUN_DIR);
                ...
                name_val = Builder.CreateGlobalStringPtr(namestr);
                acc_name_val = Builder.CreateGlobalStringPtr(acc_name);
                Value* args[] = {name_val,ktype,rdir,acc_name_val, batch,in_channels, in_height, in_width, out_channels,
				 filter_height, filter_width, zero_pad, vert_conv_stride, horiz_conv_stride, pooling,
				 pool_height, pool_width, vertical_pool_stride, horizontal_pool_stride, in, filters, out};
		...
		int tid, int num_threads)
              */
                ...
                Value *ktype, *rdir, *batch, *in_channels, *out_channels, *name_val, *acc_name_val, *in_val, *filter_val, *out_val;
                IRBuilder<> Builder(inst);
                DataLayout* dl = new DataLayout(mod);
                LLVMContext& ctx = mod->getContext();
                ...
                ktype= Builder.CreateGlobalStringPtr(KERNEL_TYPE);
		rdir = Builder.CreateGlobalStringPtr(RUN_DIR);
                in_val = (cinst->getArgOperand(5));
                filter_val = (cinst->getArgOperand(6));
                out_val = (cinst->getArgOperand(7));
                ...
                name_val = Builder.CreateGlobalStringPtr(namestr);
                acc_name_val = Builder.CreateGlobalStringPtr(acc_name);
                Value* args[] = {name_val,ktype,rdir,acc_name_val, batch, in_channels, out_channels, in_val, filter_val, out_val};
                CallInst* cI = Builder.CreateCall(print_dense_layer, args);
              } else if(f->getName().str().find("DECADES_PARTIAL_BARRIER") != std::string::npos) {
		Value *ktype, *rdir, *fence_id, *nb_threads;
                IRBuilder<> Builder(inst);
                DataLayout* dl = new DataLayout(mod);
                LLVMContext& ctx = mod->getContext();
		fence_id    = cinst->getArgOperand(0);
		nb_threads = cinst->getArgOperand(1);
                ktype= Builder.CreateGlobalStringPtr(KERNEL_TYPE);
		rdir = Builder.CreateGlobalStringPtr(RUN_DIR);
		Value* args[] = {ktype, rdir, fence_id, nb_threads};
                CallInst* cI = Builder.CreateCall(print_partial_barrier, args);
	      }
            }
          }
        }
