...
/** \brief Triming a string. Replacing boost version */ 
...
//  instr_latency[FP_REM] = 10;
  ...
  instr_latency[PARTIAL_BARRIER]=1;
  ...
  energy_per_instr[technology_node][PARTIAL_BARRIER] = 0;
...
void
Config::set_config(string parm, int val) {
  int id=param_map.at(parm); //convert text variable "param" to id corresponding to cfg variable
  set_config(id, val);
}

void Config::set_config(int id, int val) {
  switch (id) { 
  case 0:
    lsq_size = val; 
    break;
...
void Config::read(std::string name) {
  ...
  if (cfile.is_open()) {
    while (getline (cfile,line)) {
      ...
      set_config(id, stoi(s.at(0))); //set cfg variables to value in config file
