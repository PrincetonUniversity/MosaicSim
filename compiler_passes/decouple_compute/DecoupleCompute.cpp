...
namespace {
  ...
  void populate_functions(Module &M) {
    ...
    compute_consume_i8 = (Function *) M.getOrInsertFunction("desc_compute_consume_i8",
								 FunctionType::get(Type::getInt8Ty(M.getContext()), false));
  ...
  void instrument_load(Instruction *I) {
    LOADS++;
    Instruction *Intr;
    Value *Intr2;
    IRBuilder<> Builder(I);
    ...
    if (non_ptr_load(I)) {
      ...
      Intr = Builder.CreateCall(compute_consume_non_ptr_func);
      I->replaceAllUsesWith(Intr);
      to_erase.push_back(I);
    }
    else if (ptr_load(I)) {
      Intr = Builder.CreateCall(compute_consume_ptr);      
  ...
  void instrument_store(Module &M, StoreInst *I) {
    ...
    if (non_ptr_store(I)) {
      ...
      Intr = Builder.CreateCall(compute_produce_non_ptr_func, {I->getValueOperand()});
      to_erase.push_back(I);
    }
    else if (ptr_store(I)) {
  ...
  void insert_desc_main_init(Module &M, Function &f) {
    ...
    Builder.CreateCall(desc_init, {ConstantInt::get(Type::getInt32Ty(M.getContext()),COMPUTE_FLAG)});
  }
